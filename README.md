...
**29,431 lines of Rust across 13 workspace packages, 415 tests, 29 released `.deb`
...
| Key | |
|---|---|
| `B` | Split every clip under the playhead |
| `Ctrl+C` / `Ctrl+X` | Copy / cut the selected clip |
| `Ctrl+V` | Paste at the playhead |
| `Ctrl+D` | Duplicate, straight after itself |
| `Del` | Delete |
| `Ctrl+Z` / `Ctrl+Shift+Z` | Undo / redo |

Edits land on the nearest beat; **hold Alt for sample-accurate placement**.
Snapping is the default because most sessions here begin as an imported song,
where everything already sits on the grid.

**Nothing is ever written to a source file.** A clip is a window onto one — a
start, an end, and where in the file it begins reading — so trimming and
splitting only move that window. Clips split from one take share a single
decoded copy in the cache, which is why a split is instant and costs no memory,
and why the two halves played back to back reproduce the take exactly. Trimming
the front walks the window forward with the edge, so the audio stays where it
was instead of sliding under the cursor, and an edge pulled back out again
recovers what was hidden.

An edge can only ever show audio the file actually has: dragging the end
outwards stops at the last sample rather than leaving a clip longer than
anything it can play.
...
The **LOOP** readout in the transport bar shows the range the audio engine is
actually holding, read back from it rather than from the interface — so what is
on screen is what the callback will act on. Clicking it clears the loop.
...
Marking a loop arms it and moves the playhead to its start; press Play to hear
it. Looping wraps in the audio callback on a comparison against a range read
once per callback — no allocation, no lock. It lands within one render chunk of
the mark rather than on the exact sample; making it sample-accurate means
splitting the block, which is not worth the complexity until someone can hear
it.
...
The chart is drawn above the tracks, on the same grid and the same scroll as
the audio, so a chord sits over the bar it belongs to. One cell per quarter
note, and a chord is printed only where it **changes** — a dot means it is
still playing, the way a lead sheet is written:
...
Repeating `C#m C#m C#m C#m` would be noise on the page; the eye is looking for
the changes anyway. Bar lines are drawn at each downbeat, and a chord the
detector is unsure of is printed dimmed rather than hidden.

**Scroll over the lane to resize it.** Reading a chart is something people do
at a glance from across a room, sometimes with a guitar in their hands, so how
big it wants to be is a matter of the room and the eyes rather than anything
the application can pick correctly on anyone's behalf. The size is remembered
in `~/.config/rustdaw/audio.json` and written once the wheel comes to rest.

Resolving onto the beat grid is a correctness fix as much as a display one: a
span that begins 40 ms before the beat is describing the analysis rather than
the music, and on the grid it lands where the band played it. It merges only
2–8% of spans across the local corpus, so the honest claim is that it fixes
*placement* and readability, not the number of chords found.

Chords and the detected key are now kept in the session. They were being
detected at import, written to the session file, and then discarded the first
time the session was saved from the application, which meant the chart quietly
disappeared.
...
| [`apps/rustdaw`](apps/rustdaw) | 6,767 | The egui desktop application: timeline, Smart Tool editing, mixer, piano roll, tuner, chord lane |
| [`crates/daw-audio-linux`](crates/daw-audio-linux) | 4,069 | The real-time runtime — PipeWire/Pulse via cpal, lock-free command and metering channels, disk writers, time stretching |
...
| [`crates/daw-project`](crates/daw-project) | 919 | The versioned session document, its migrations, clip editing, and the chord chart |
...
**415 test functions.** The product principles the whole thing was built against
...
the not-a-knot spline and the LOWESS smoother, the 415 tests. Held 27,000 lines of Rust in view at once and kept the real-time
...
Twenty-nine packages, all of them in [`dist/`](dist/), all of them installable.
...
- **11 crates and 2 applications, 29,431 lines of Rust**, `unsafe` forbidden at
  the workspace level, `clippy::pedantic` clean, 415 tests.
...
- **Twenty-nine installable packages** and a macOS bundle script.
